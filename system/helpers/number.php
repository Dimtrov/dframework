...
/**
 * dFramework Number Helpers
 *
 * @package		dFramework
 * @subpackage	Helpers
 * @category	Number
 * @since 		3.3.0
 * @credit      CodeIgniter Number Helper
 */
...
if (! function_exists('number_to_size'))
{
	...
	function number_to_size($num, int $precision = 1, string $locale = null)
	{
		...
		if (! empty($locale) AND ( $underscorePos = strpos($locale, '_')))
		{
...
if (! function_exists('format_number'))
{
	...
	function format_number(float $num, int $precision = 1, string $locale = null, array $options = []): string
	{
		...
		$locale = $locale ?? \dFramework\core\loader\Service::request()->getLocale();
