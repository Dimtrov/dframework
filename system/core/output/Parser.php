...
class Parser extends View
{
	...
	protected function makeView(string $view, array $options = null, ?string $viewPath = null) : string
    {
		...
		if (isset($options['cache']))
		{
			...
			if ($output = Service::cache()->read($cacheName))
			{
		...
		if (!is_file($file))
		{
            throw new LoadException('Le fichier "'.$file.'" n\'exite pas', 404);
		}
	...
	protected function parseConditionals(string $template): string
	{
		...
		{
			ob_end_clean();
			throw new Exception('You have a syntax error in your Parser tags: '. str_replace(['?>', '<?php '], '', $template));
		}
		return ob_get_clean();
	}
	 ...
	 * @return View
	 */
	public function setDelimiters($leftDelimiter = '{', $rightDelimiter = '}') : View
	{
