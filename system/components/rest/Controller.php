...
class Controller extends dF_Controller
{
    ...
    */
    ...
    protected function _log_access_time()
    {
        ...
        try {
            $this->rest->db->query('UPDATE '.(($reest->db->db->config['prefix'] ?? '').$this->_config['rest_logs_table']).' SET rtime = :rtime WHERE id = :id', [
                'id' => $this->_insert_id,
    ...
    protected function _log_response_code($http_code)
    {
        ...
        try {
            $this->rest->db->query('UPDATE '.(($reest->db->db->config['prefix'] ?? '').$this->_config['rest_logs_table']).' SET response_code = :response_code WHERE id = :id', [
                'id' => $this->_insert_id,
